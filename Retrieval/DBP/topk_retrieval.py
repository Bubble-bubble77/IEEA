...
class Retrieval():
    ...
    def Bi_direct_retrieval2(self, k, side=None):
        def dev_pair_load():
            filepath = f'../data/DBP15K/DBP_1/'
            filename = os.path.join(filepath, self.dataset, 'dev_ent_ids')
            dev_pair = []
        ...
        if side is not None:
            file_path = f'Fuse_{self.dataset}_N.npy'
        else:
            file_path = f'Fuse_{self.dataset}.npy'
        print(file_path)
        ...
        folder_path = '/topk_id_score'
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)

        if side is not None:
            file_name = os.path.join(folder_path,
                                     f'{self.dataset}_top{k}_Fuse_Bidirect_{side}.npy')
        else:
            file_name = os.path.join(folder_path, f'{self.dataset}_top{k}_Fuse_Bidirect_Sinkhorn.npy')
...
re1 = Retrieval('zh_en')
re1.Bi_direct_retrieval2(10, side='N')
re1.Bi_direct_retrieval2(10, side=None)
print('zh_en end!')

re2 = Retrieval('ja_en')
re2.Bi_direct_retrieval2(10, side='N')
re2.Bi_direct_retrieval2(10, side=None)
print('ja_en end!')

re3 = Retrieval('fr_en')
re3.Bi_direct_retrieval2(10, side='N')
re3.Bi_direct_retrieval2(10, side=None)
print('fr_en end!')