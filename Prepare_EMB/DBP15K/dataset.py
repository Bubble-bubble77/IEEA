...
class DBPDataset():
    def __init__(self, file_path, device):
        ...
        self.test_pair = self.load_alignment_pair(file_path + '/ill_ent_ids')

        self.index_1, self.index2 = self.load_index(file_path + '/index_1'), self.load_index(file_path + '/index_2')
        self.kg1_num = len(self.index_1)
        ...
        # modify
        self.val_pair = self.load_alignment_pair(file_path + '/dev_ent_ids')
        self.val_pair = np.array(self.val_pair)
        # self.val_pair = torch.tensor(self.val_pair)
        self.rest_pair = self.val_pair.copy().T
    ...
    def load_index(self, path):
        ...
        with open(path, 'r') as f:
            for line in f:
                ...
                index[ent_id] = int(ent_index)
        return index

    def pair_trans_by_index(self, pairs, index_1, index_2):
        new_pair = []
        for p in pairs:
            p1, p2 = p
            new_p1 = index_1[str(p1)]
            new_p2 = index_2[str(p2)]
            new_pair.append((new_p1, new_p2))
        return new_pair
